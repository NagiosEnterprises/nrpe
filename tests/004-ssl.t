...
launch_daemon("--config configs/ssl.cfg");


@output = `$checknrpe -H 127.0.0.1 -p 40321 --ca-cert-file=configs/nrpe.crt 2>&1`;
is($?, STATE_OK, 'connect ssl') || diag @output;
like($output[0], qr/NRPE v.*/, 'connect ssl response') || diag @output;

@output = `$checknrpe -H 127.0.0.1 -p 40321 --ca-cert-file=configs/nrpe.crt --client-cert=configs/nrpe.crt --key-file=configs/nrpe.key 2>&1`;
